...
let menuService = {
    ...
    saveMenu: async (menu) => {
        let db = await model.init(context)
        let exist = await db.find({ name: menu.name }).value()
    ...
    delMenu: async (id)=> {
        let db = await model.init(context);
        let exist = db.find({id}).value();
        let removeId = id;
        let removePids = [removeId];
        if(!exist){
            return {
                success:false,
                msg:"不存在删除的菜单"
            ...
            while(removeId=removePids.shift()){
                let {id:curId} = await db.find({ parentId: removeId }).value()||{};
                if(!curId){
                    continue;
                }
                console.log("删除项目的id",curId);
                removePids.push(curId);
                await db.remove({ parentId: removeId }).write()
            }
